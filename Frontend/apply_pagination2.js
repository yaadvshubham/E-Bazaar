...
const startSlice = script.indexOf('    // Render');
const endSlice = script.indexOf('  }\r\n\r\n  if (minInput)');
if (startSlice === -1 || endSlice === -1) {
    // try with \n
    const endSlice2 = script.indexOf('  }\n\n  if (minInput)');
    if (startSlice === -1 || endSlice2 === -1) {
        console.error("Could not find start/end slices for replacing renderFilteredProducts logic.");
        process.exit(1);
    }
}

let actualEnd = script.indexOf('  }\r\n\r\n  if (minInput)');
if (actualEnd === -1) actualEnd = script.indexOf('  }\n\n  if (minInput)');

const newLogic = `    // Render with Pagination
...
let afterLogic = script.substring(actualEnd);

// Perform event listener replacements safely in afterLogic
...
afterLogic = afterLogic.replace(/addEventListener\('click', \(\) => \{\n\s*document.querySelectorAll/g, "addEventListener('click', () => {\n    window.currentPage = 1;\n    document.querySelectorAll");
...
console.log('script.js pagination logic fully applied!');
