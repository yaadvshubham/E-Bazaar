...
css = re.sub(
    r'\.brands-grid\{display:grid;grid-template-columns:repeat\(\d+,1fr\);gap:16px\}',
    '.brands-grid{display:flex;overflow-x:auto;scroll-snap-type:x mandatory;gap:16px;padding-bottom:16px;scrollbar-width:thin;}',
    css
)

# Update .brand-tile to have fixed width
css = re.sub(
    r'\.brand-tile\{',
...
css = re.sub(r'^\s*\.brands-grid\{grid-template-columns:repeat.*?\}.*?$', '', css, flags=re.MULTILINE)
...
new_brands = '''    <div class="brands-grid" role="list">
...
start_idx = html.find('<div class="brands-grid" role="list">')
end_idx = html.find('</section>', start_idx)

if start_idx != -1 and end_idx != -1:
    html = html[:start_idx] + new_brands + '\n  ' + html[end_idx:]
    with open('index.html', 'w', encoding='utf-8') as f:
        f.write(html)
    print('Updated index.html')
else:
    print('Failed to find HTML markers')
